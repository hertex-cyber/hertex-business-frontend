...
import { MEETING_STATUS_OPTIONS, MEETING_STATUS_STYLES } from '../constants';

const MeetingCard = ({ event, onClick }) => {
  const statusStyle = MEETING_STATUS_STYLES[event.status] || MEETING_STATUS_STYLES.upcoming;
  return (
    <div className={cn("p-3 rounded-xl space-y-1.5 cursor-pointer transition-all", event.status === 'live' ? "bg-emerald-500/10 border border-emerald-500/30 hover:bg-emerald-500/15" : "bg-white/[0.06] border border-white/10 hover:bg-white/[0.09]")} onClick={onClick}>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {event.status && (
            <span className={cn("shrink-0 px-1.5 py-0.5 rounded-full border text-[8px] font-bold uppercase tracking-wider", statusStyle)}>
              {MEETING_STATUS_OPTIONS.find(o => o.value === event.status)?.label || event.status}
            </span>
          )}
