...
const FollowUpCard = ({ event }) => {
  return (
    <div className="p-3 bg-white/[0.06] border border-white/10 rounded-xl space-y-1.5">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <span className="shrink-0 w-6 h-6 rounded flex items-center justify-center border text-amber-400 bg-amber-500/10 border-amber-500/20">
            <Bell size={10} />
          </span>
          <p className="text-sm font-bold text-white leading-tight truncate">{event.title}</p>
        </div>
        <span className="shrink-0 px-1.5 py-0.5 rounded-full border text-[8px] font-bold uppercase tracking-wider text-amber-400 bg-amber-500/10 border-amber-500/20">Follow Up</span>
      </div>
